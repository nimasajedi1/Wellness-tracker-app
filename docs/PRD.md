...
**DEVICE-007 [P2] iOS 27 native nutrition-label understanding.** On iOS 27-capable devices, add an enhanced label-capture path using Apple's public native frameworks rather than a custom or paid vision model. Capture remains user initiated. Run Vision `RecognizeDocumentsRequest` first to extract the document hierarchy, including text, tables, lists, and machine-readable codes; Apple explicitly identifies nutritional labels as a supported structured-document use case. Where Apple Intelligence / Foundation Models image prompting is available, the app MAY pass the captured label image and/or Vision result to the Foundation Models framework, with Vision `OCRTool` and `BarcodeReaderTool` available to the model, to map the label into the app-owned structured nutrition schema. The model is an interpreter only: deterministic code validates units, serving basis, nutrient names, numeric bounds, Daily Value versus absolute amount, and arithmetic before anything can be committed. [S21-S23]

The iOS 27 path MUST produce a reviewable draft containing, when present and legible: product/label name, servings per container, serving-size description, serving mass/volume, calories, total fat, saturated fat, trans fat, cholesterol, sodium, total carbohydrate, dietary fiber, total sugars, added sugars, protein, and declared vitamins/minerals. Preserve both the normalized value and enough source evidence to show the user what was read. Missing, unreadable, conflicting, or unsupported fields remain unknown; they are never inferred merely to complete the schema. Percent Daily Value is not substituted for grams/milligrams. Multiple nutrition columns or bases (for example per serving versus per container, or prepared versus unprepared) must remain distinct until the user selects the intended basis.

The review screen MUST show the captured image or relevant crop beside the extracted values, highlight low-confidence/ambiguous fields, allow correction of every field, and require explicit confirmation before creating or updating a food. A label scan cannot silently overwrite a verified catalog food. After confirmation, store a snapshot of the accepted nutrition facts so historical diary totals do not change when a later scan or catalog version differs.

This is an enhancement, not a new minimum OS requirement. The project keeps its existing baseline unless separately changed: devices/OS versions without the iOS 27 APIs continue to use `DEVICE-002` native text recognition plus manual review. Foundation-model unavailability, Apple Intelligence being disabled, unsupported hardware, model refusal, OCR failure, or network unavailability MUST degrade to deterministic Vision/manual capture rather than blocking food entry. The default path does not upload the nutrition-label image to the app backend. Private Cloud Compute or any cloud model is outside this label-scanning path unless a later requirement explicitly adds it with separate privacy/consent review.

**DEVICE-007 acceptance criteria.** On a supported iOS 27 device, a user can photograph a standard Nutrition Facts panel and receive a structured, editable draft without a paid API or app-hosted cloud inference. Tests MUST cover: a clean single-column US label; a bilingual/visually complex label where supported by Vision recognition languages; per-serving and per-container columns; a label containing both absolute nutrient values and %DV; a partially obscured/blurred label; an unsupported or missing nutrient; an image that is not a nutrition label; model unavailable with Vision/manual fallback; and correction before commit. Evaluation records extraction accuracy separately from commit correctness. No test passes merely because a generative model returned valid JSON: the accepted values must survive deterministic validation and user confirmation.
...
- **[S21] Vision structured document recognition.** `https://developer.apple.com/documentation/vision/recognizedocumentsrequest` — document hierarchy extraction (text, tables, lists, machine-readable codes), with nutrition labels named as a structured-document use case. Verify the exact API surface against the installed iOS 27 SDK before use.
- **[S22] Foundation Models image prompting.** `https://developer.apple.com/documentation/foundationmodels` — passing an image alongside a prompt for on-device structured generation, where the installed SDK and device support it. Availability, prompt shape, and safety behavior must be rechecked against the shipped iOS 27 SDK; this PRD does not assume the beta surface is final.
- **[S23] Vision tools for Foundation Models.** `https://developer.apple.com/documentation/foundationmodels/expanding-generation-with-tool-calling` — exposing Vision `OCRTool` and `BarcodeReaderTool` to model generation as read-only tools; confirm the tool names and signatures in the iOS 27 SDK.
